...
# RHEL-6 only supports x86_64
ExcludeArch: i686 s390x ppc64
...
BuildRequires: %{?scl_prefix}gcc %{?scl_prefix}elfutils %{?scl_prefix}elfutils-devel %{?scl_prefix}elfutils-libelf-devel rpm-devel %{?scl_prefix}binutils-devel
...
# RHEL-6 does not define %%make_build ...
# Also on RHEL-6 glibc does not provide the real time clock functions,
# so these have to be specifically included via the rt library.
make LIBS="-lrt"
...
%install
# RHEL-6 does not define %%make_install ...
